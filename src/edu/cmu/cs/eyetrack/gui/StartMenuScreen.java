...
public class StartMenuScreen extends Screen {
	...
	private JRadioButton rdoMemCheckNone, rdoMemCheck2x2, rdoMemCheckNxN, rdoMotionPixel, rdoMotionGrid, rdoMotionLinear, rdoMotionRandom;
	...
	protected void initialize() {
		...
		cbxTargetType = new JComboBox(targetTypes);
		cbxTargetType.setSelectedIndex((new Random()).nextInt(targetTypes.length));
		...
		JLabel lblRandomTarget = new JLabel("Target:", JLabel.TRAILING);
		...
		if(Util.CMU_ONLY) {
			trialDataPanel.add(lblRandomTarget); trialDataPanel.add(chkRandomTarget); numRows++;
			trialDataPanel.add(new JPanel(new GridLayout(0,1)){{ add(lblTargetType); add(lblTargetColor);}}); trialDataPanel.add(new JPanel(new GridLayout(0,1)){{ add(cbxTargetType); add(cbxTargetColor);}}); numRows++;
		}
