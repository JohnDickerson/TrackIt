...
public class PilotOutputRecord {
	...
	public void writeToCSV(String filename) throws IOException {
		...
		for(TRIAL_TYPE trialType : dataMap.keySet()) {
			...
			int extraColCount = 3;
			int numCSVColumns = 1 + trialIDSet.size() + extraColCount*(1 + trialIDSet.size());
			...
			// Third columns correspond to number of fixation points
			// Fourth columns correpond to number of fixation points on a distractor, not on a target
			for(int extraColIdx=0; extraColIdx<extraColCount+1; extraColIdx++) {
				for(Integer trialID : trialIDSet) {
					header[idx++] = "Trial " + trialID;
				}

				if(extraColIdx<extraColCount) {
					header[idx++] = "";
				}
			}
			...
			for(String subjectID : trialMap.keySet()) {
				...
				// ... And the number of fixation points for this trial
				dataRow[idx++] = "";
				for(Integer trialID : records.keySet()) {
					long fixationPtCount = records.get(trialID).getFixationPointCount();
					dataRow[idx++] = String.valueOf(fixationPtCount);
				}
				// ... And the number of fixation points on a distractor and not the target
				dataRow[idx++] = "";
				for(Integer trialID : records.keySet()) {
					long fixationPtOnDistractorCt = records.get(trialID).getFixationNearDistractorNotTargetCount();
					dataRow[idx++] = String.valueOf(fixationPtOnDistractorCt);
				}
